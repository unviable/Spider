...
import com.woniu.util.HttpUtil;
import com.woniu.util.LoadImg;
import com.woniu.util.LoadVideo;


public class Spider {	
	public static String path = "http://gaoxiao.52op.net/fangyan/6195.htm";	//雅达公司官网
	...
	public static void getAllLinks(String path){
		...
		for (Element element : videos) {
			...
			if(!videourl.contains("http://")&&!videourl.contains("https://")){
				...
				//否则404
				videourl = Spider.path+videourl;
			}
			if(videourl.contains(".mp4")) {
				try {
					LoadVideo.getDondow(videourl, "D:\\Spider\\Videos\\"+h1.text()+".mp4");
				} catch (Exception e) {
			...
			if(!readTxtFile(aLinkFile).contains(url)
				...
				if(url.contains(Spider.path)){		
					...
					if(url.contains(".doc")||url.contains(".exl")
						...
						try {
							LoadImg.downloadPicture(url, "D:\\Spider\\Docs\\"+s);
						} catch (Exception e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
		...
		for(Element element:imgLinks){
			...
			if(!readTxtFile(imgLinkFile).contains(srcStr)){	
				...
				String s = srcStr.substring(srcStr.lastIndexOf('/')+1, srcStr.length());
				try {
					LoadImg.downloadPicture(srcStr, "D:\\Spider\\Imgs\\"+s);
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
